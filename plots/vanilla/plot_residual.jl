...
for (N_i, N) in enumerate(Ns)
  for (alpha_i, alpha) in Iterators.reverse(enumerate(alphas))
    ...
    plot!(1:STRIDE:length(residuals)*STRIDE, residuals, fmt = :png, labels=[L"N = %$(N), \alpha = %$(alphas_text[alpha_i])"], yaxis=:log, color=colors[N_i], linestyle=linestyles_text[alpha_i])
  end
end
...
for (N_i, N) in enumerate(Ns)
  for (alpha_i, alpha) in enumerate(alphas)
    ...
    plot!(1:STRIDE:length(residuals)*STRIDE, residuals, fmt = :png, labels=[L"N = %$(N), r = %$(alpha)"], yaxis=:log, color=colors[N_i], linestyle=linestyles[alpha_i])
  end
end
